...
public class DocController {
     ...
     * 忽略添加属性的类型
     */
    private static final List<Class<?>> IGNORE_PROPERTY_TYPES = Arrays.asList(
            String.class,
            Collection.class,
            Map.class
    );
    ...
    private Api getApi(ApiHandler apiHandler) {
        ...
        Class<?> apiClass = apiHandler.getMethod().getDeclaringClass();
    ...
    private List<Property> getProperties(Type type, Set<String> recurredTypeNames) {
        ...
        if (type instanceof Class) {
            ...
            //忽略的类型(及其子类)直接返回
            for (Class<?> ignoreType : IGNORE_PROPERTY_TYPES) {
    ...
    private List<Property> getMapProperties(ParameterizedType parameterizedType, Set<String> recurredTypeNames) {
        ...
        if (keyClass != null && keyClass.isAnnotationPresent(OpenApiDoc.class)) {
            OpenApiDoc apiDoc = keyClass.getAnnotation(OpenApiDoc.class);
            property.setCnName(apiDoc.cnName());
            property.setDescribe(apiDoc.describe());
        }
        properties.add(property);
